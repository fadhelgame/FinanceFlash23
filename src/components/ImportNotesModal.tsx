...
import { Sparkles, CheckSquare, Square, ArrowLeft } from 'lucide-react'
...
export default function ImportNotesModal({
  ...
  const handleAdd = () => {
    ...
    const transactions: Transaction[] = chosen.map(t => ({
      id: generateId(),
      title: t.title,
      amount: t.amount,
      category: t.category,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div
        className="relative w-full max-w-lg card rounded-t-3xl sm:rounded-3xl p-6 max-h-[90vh] overflow-y-auto animate-slide-up"
        style={{ background: 'var(--color-paper-0)' }}
      >
        <div className="flex items-center gap-2 mb-1">
          {results && (
            <button onClick={() => setResults(null)} style={{ color: 'var(--color-ink-2)' }}>
              <ArrowLeft className="w-4 h-4" />
            </button>
          )}
          <Sparkles className="w-4 h-4" style={{ color: 'var(--color-accent)' }} />
          <h2 className="text-base font-semibold" style={{ color: 'var(--color-ink-0)' }}>
            {results ? 'Review' : 'Import from notes'}
          </h2>
        </div>
        ...
        {!results ? (
          <>
            <p className="text-xs mb-4" style={{ color: 'var(--color-ink-2)' }}>
              Paste your notes. They get read into transactions you can check before anything is added.
            </p>
            <textarea
              value={text}
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={handleClose} className="btn-ghost flex-1 py-3">
                Cancel
              </button>
              <button
                onClick={handleParse}
              ...
              {results.map((t, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-semibold shrink-0 ${t.isIncome ? 'text-income' : 'text-expense'}`}
                    >
                      {t.isIncome ? '+' : '-'}
                      {formatIDR(t.amount)}
                    </span>
